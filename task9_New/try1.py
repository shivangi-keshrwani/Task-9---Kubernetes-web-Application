...
lined = t.split()
if lined[0] == "1":
    deployment_name = lined[2]
    image_name = lined[1]
    cmd = "kubectl create deployment " + \
        (deployment_name) + " --image=" + (image_name)
    ...
    port_no = lined[2]
    Expose_type = lined[3]
    cmd = "kubectl expose deployment " + \
        (deployment_name) + " --port=" + (port_no) + " --type=" + (Expose_type)
    output = subprocess.getoutput("sudo " + cmd + " --kubeconfig admin.conf")
    print(output)
elif lined[0] == "6":
    deployment_name = lined[1]
    ...
    cmd = "kubectl scale deployment " + \
        (deployment_name) + " --replicas=" + (replica)
    output = subprocess.getoutput("sudo " + cmd + " --kubeconfig admin.conf")
    print(output)
elif lined[0] == "7":
